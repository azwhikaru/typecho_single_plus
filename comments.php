...
?>
    ...
    <div class="comment-single<?php echo $commentLevelClass; ?>" id="<?php $comments->theId() ?>">
        ...
        <div class="comment-meta">
            <span class="comment-author"><?php if ($comments->url) : ?><a href="<?php $comments->url() ?>" rel="external nofollow" target="_blank"><?php $comments->author(false) ?></a><?php else : $comments->author();
                                                                                                                                                                                        endif; ?></span>
...
<section class="post-comments" id="comments">
    <h3><?php $this->commentsNum(_t('没有评论'), _t('共有 %d 条评论'), _t('共有 %d 条评论')); ?></h3>
            ...
            <form method="post" action="<?php $this->commentUrl() ?>" role="form">
                <?php if ($this->user->hasLogin()) : ?>
                    <fieldset>
                        ...
                        <textarea rows="2" name="text" id="textarea" placeholder="<?php echo $this->options->custom_comment_hint ? $this->options->custom_comment_hint : "在这里输入评论" ?>" required><?php $this->remember('text'); ?></textarea>
                ...
                <?php else : ?>
                    <div class="row">
                        ...
                        <fieldset class="col-m-6">
                            <textarea rows="3" name="text" id="textarea" placeholder="<?php echo $this->options->custom_comment_hint ? $this->options->custom_comment_hint : "在这里输入评论" ?>" required><?php $this->remember('text'); ?></textarea>
                            <button type="submit" class="btn">提交</button>
                        </fieldset>
                    </div>
                <?php endif; ?>
